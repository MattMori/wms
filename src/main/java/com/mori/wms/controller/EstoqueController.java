...
public class EstoqueController {
    ...
    public ResponseEntity<String> adicionarEstoque(@RequestBody MovimentacaoDTO dto) {
        ...
        if (dto.getCodigoEtiqueta() != null && !dto.getCodigoEtiqueta().isEmpty()) {
            rack = rackRepository.findByCodigoEtiqueta(dto.getCodigoEtiqueta())
                    .orElseThrow(() -> new RuntimeException("Etiqueta de Rack não encontrada: " + dto.getCodigoEtiqueta()));
    ...
    public ResponseEntity<String> registrarSaida(@RequestBody MovimentacaoDTO dto) {
        ...
        // Acha o rack pelo ID (ou podemos mudar para etiqueta no DTO também se preferir)
        // Vamos assumir que aqui o front já pegou o ID do rack na consulta anterior
        
        // Busca o produto pelo SKU
        Produto produto = produtoRepository.findBySku(dto.getSku())
                .orElseThrow(() -> new RuntimeException("Produto não encontrado: " + dto.getSku()));

        // Busca o estoque EXATO daquele produto naquele rack
        // (Aqui vamos simplificar pegando o primeiro lote que achar, mas num WMS real filtra por lote)
        ...
        if (novaQuantidade < 0) {
            ...
            // A MÁGICA: Se zerou, deleta a linha do banco!

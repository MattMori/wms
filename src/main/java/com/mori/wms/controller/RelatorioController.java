...
public class RelatorioController {
        ...
        public ResponseEntity<DashboardDTO> getDashboard() {
                ...
                long capacidadeTotalCaixas = rackRepository.findAll().stream()
                                .mapToLong(r -> r.getCapacidadeMaxCaixas())
                                .sum();
        ...
        public ResponseEntity<String> getOcupacaoRack(@PathVariable Long rackId) {
                ...
                long caixasNoRack = estoqueRepository.findByRackId(rackId).stream()
                                .mapToLong(e -> e.getQuantidade()
                                                / (e.getProduto().getItensPorCaixa() > 0
                                                                ? e.getProduto().getItensPorCaixa()
                                                                : 1))
                                .sum();

                double porcentagem = ((double) caixasNoRack / rack.getCapacidadeMaxCaixas()) * 100;

                return ResponseEntity.ok(String.format(
                                "Rack: %s | Ocupação: %d/%d Caixas (%.1f%%)",
                                rack.getCodigoEtiqueta(), caixasNoRack, rack.getCapacidadeMaxCaixas(), porcentagem));
        }
}